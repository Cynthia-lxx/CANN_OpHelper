...
#: Big ANSI-Shadow-style banner for the PASS verdict -- the same look as classic
#: figlet "ANSI Shadow" output (glyphs cut from the public-domain ANSI Shadow
#: font, 6 visible rows tall). Rendered at generation time and embedded as plain
#: text into the generated verify_result.py, so that file keeps zero runtime
#: logic beyond print().
_FONT_ROWS = 6
_FONT = {
    "A": (
...
def _banner(text: str) -> str:
    """Render ``text`` as one horizontal ANSI-Shadow-style block banner.

    Each glyph keeps a one-blank-column right padding and spaces become a wider
    word gap, so letters never touch. Every character (except space) must have a
    glyph in :data:`_FONT`; unknown chars raise :class:`ValueError`.
    """
    unknown = sorted({ch for ch in text.replace(" ", "")} - set(_FONT))
    if unknown:
        raise ValueError(f"banner glyph not in font: {''.join(unknown)}")
    widths = {ch: max(len(row) for row in _FONT[ch]) + 1 for ch in _FONT}
    lines = []
    for row in range(_FONT_ROWS):
        ...
        for ch in text:
            if ch == " ":
                ...
                line += _FONT[ch][row].ljust(widths[ch])
...
def verify_result():
    ...
    if not bad:
        print('''__BANNER_HOLY__''')
        print('''__BANNER_PASS__''')
        print("TEST PASSED!")
        return True
    print("TEST FAILED!")
...
""".replace("__WIDTH__", str(width))
    .replace("__FMT__", fmt_char)
    .replace("__BANNER_HOLY__", _banner("HOLY SHIT"))
    .replace("__BANNER_PASS__", _banner("TEST PASS")))
