...
_ZH: Dict[str, str] = {
    ...
    "fillgen.err.dtype_v1": "表达式算子 dtype 仅支持 float（float32）/float16。当前声明：{values}",
    "fillgen.err.dtype_v1.hint": "仅支持 ND 格式逐元素算子；int 等留待后续迭代（见 docs/expr-rules.md §5）",
    "fillgen.err.shape_conflict": "各张量的 shape 提示不一致，无法确定验证元素数",
    "fillgen.err.shape_dynamic": "shape 提示含动态维（-1），无法用于验证",
    "fillgen.err.shape_dynamic.hint": "验证需要确定形状；去掉 shape 提示可回退到默认元素数",
...
_EN: Dict[str, str] = {
    ...
    "fillgen.err.dtype_v1": "Expression dtypes support only float (float32)/float16. Declared: {values}",
    "fillgen.err.dtype_v1.hint": "ND element-wise only; int types are future work (see docs/expr-rules.md §5)",
    "fillgen.err.shape_conflict": "Tensor shape hints disagree, so the verification element count is ambiguous",
    "fillgen.err.shape_dynamic": "Shape hints contain dynamic (-1) dims and cannot drive verification",
    "fillgen.err.shape_dynamic.hint": "Verification needs a concrete shape; dropping the shape hints falls back to the default element count",
